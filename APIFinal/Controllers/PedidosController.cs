...
namespace APIFinal.Controllers
{
    ...
    public class PedidosController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddProductoToPedido(int pedidoId, [FromBody] Pedido_ProductoModel request)
        {
            request.PedidoId = pedidoId;
            Pedido_ProductoModel response = await pedidosLogic.AddProductoToPedido(request);
            return Ok(response);
        }
    }
}
